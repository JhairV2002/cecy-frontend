...
import { AttendanceModel, RegistrationModel } from '@models/cecy';
...
export class AttendanceListComponent implements OnInit {
  ...
  getAttendances() {
    this.api
      .getAttendanceDetailPlanification(this.attendanceId)
      .subscribe((response) => {
  ...
  redirectParticipants(attendanceId: number) {
    this.router.navigate([
      '/cecy/instructor/participant-attendance/',
      attendanceId,
    ]);
  }
  ...
  redirectPhotograficRecords() {
    this.router.navigate([
      '/cecy/instructor/attendance-records/',
      this.attendanceId,
    ]);
  }

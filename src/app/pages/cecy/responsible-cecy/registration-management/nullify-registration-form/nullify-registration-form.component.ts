...
registrationId:number;
  ...
  constructor(
    ...
    this.registration$.pipe(takeUntil(this.unsubscribe$))
    .subscribe(response => {
      // Llena el formulario
      if (response.id !== undefined) {
        this.customNullification.id = response.id;
        this.customNullification.email = response.participant.user.email;
        this.customNullification.lastname = response.participant.user.lastname;
        this.customNullification.name = response.participant.user.name;
        this.customNullification.username = response.participant.user.username;
  ...
  confirmNullify(): void {
    this.messageService.questionNullify({})
      .then((result) => {
        if (result.isConfirmed) {
          this.registrationHttpService.nullifyRegistration(this.registrationId, this.formNullifyRegistration.value).subscribe(
            response => {

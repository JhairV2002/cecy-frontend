...
import { DetailPlanificationModel, PaginatorModel, PlanificationModel, RegistrationModel, SchoolPeriodModel } from '@models/cecy';
...
/*DDRC-C: observables */
    ...
    this.messageService.questionDelete({})
      .then((result) => {
        if (result.isConfirmed) {
          this.registrationHttpService.eliminate(registration.id!)
            .subscribe(
              response => {
  ...
  reEnroll(registration: RegistrationModel): void {
    this.messageService.questionReenroll({})
      .then((result) => {
        if (result.isConfirmed) {
          this.registrationHttpService.reEnroll(registration.id!,registration)
          .subscribe(
            response => {
                this.messageService.success(response);
              },
  ...
  nullifyRegistrations() {
    ...
    this.registrationHttpService.nullifyRegistrations(this.customPlanificationList).subscribe(
      response => {    
        this.messageService.success(response);
      },
      error => {
  ...
  goToRegistrationForm(action:any,registration: RegistrationModel = {}, ){
    this.router.navigate([`/cecy/responsible-cecy/registration/${this.idDetailPlanification}/${action}/${registration.id}`])
  }

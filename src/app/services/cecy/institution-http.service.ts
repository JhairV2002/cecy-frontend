...
export class InstitutionHttpService {
  ...
  storeInstitution(institution: InstitutionModel): Observable<ServerResponse> {
    ...
    this.loaded.next(true);
    return this.httpClient.post<ServerResponse>(url, institution).pipe(
      map((response) => response),
      tap(
        (response) => {
          this.loaded.next(false);
  ...
  updateInstitution(
    ...
    this.loaded.next(true);
    return this.httpClient.put<ServerResponse>(url, institution).pipe(
      map((response) => response),
      tap(
        (response) => {
          this.loaded.next(false);
          const index = this.institutionsList.data.findIndex(
            (institution: InstitutionModel) =>
              institution.id === response.data.id
          );
  ...
  deleteInstitution(id: number): Observable<ServerResponse> {
    ...
    return this.httpClient.delete<ServerResponse>(url).pipe(
      ...
      tap(
        (response) => {
          this.loaded.next(false);
          this.institutionsList.data = this.institutionsList.data.filter(
            (institution: InstitutionModel) =>
              institution.id !== response.data.id
          );
          this.institutions.next(this.institutionsList);

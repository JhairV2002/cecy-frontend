...
export class InstructorHttpService {
  ...
  private instructorModel: any = {};
  private instructor = new BehaviorSubject<any>({});
  ...
  storeInstructor(instructor: any): Observable<ServerResponse> {
    const url = `${this.API_URL_PRIVATE}/create`;

    this.loaded.next(true);
    return this.httpClient.post<ServerResponse>(url, instructor).pipe(
      map((response) => response),
      tap(
        (response) => {
          this.loaded.next(false);
  ...
  updateInstructor(id: number, instructor: any): Observable<ServerResponse> {
    const url = `${this.API_URL_PRIVATE}/${id}/state-type`;

    this.loaded.next(true);
    return this.httpClient.put<ServerResponse>(url, instructor).pipe(
      map((response) => response),
      tap(
        (response) => {
          this.loaded.next(false);
          const index = this.instructorsList.data.findIndex(
            (instructor: any) => instructor.id === response.data.id
          );
  ...
  deleteInstructor(id: number): Observable<ServerResponse> {
    ...
    return this.httpClient.delete<ServerResponse>(url).pipe(
      ...
      tap(
        (response) => {
          this.loaded.next(false);
          this.instructorsList.data = this.instructorsList.data.filter(
            (instructor: any) => instructor.id !== response.data.id
          );
          this.instructors.next(this.instructorsList);
        },
  ...
  storeInstructors(users: number[]) {
    ...
    return this.httpClient.post<ServerResponse>(url, { ids: users }).pipe(
      tap(
        (response) => {
          this.loaded.next(false);
          const index = this.instructorsList.data.findIndex(
  ...
  mapInstructors(instructors: any[]) {

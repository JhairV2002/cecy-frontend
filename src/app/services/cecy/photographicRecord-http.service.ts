...
export class PhotographicRecordHttpService {
  ...
  storePhotographicRecord(photographicRecord: PhotographicRecordModel): Observable<ServerResponse> {
    const url = `${this.API_URL_PRIVATE}`;
    this.loaded.next(true);
    return this.httpClient.post<ServerResponse>(url, photographicRecord)
      .pipe(
        map(response =>{
  ...
  updatePhotographicRecord(id: any, photographicRecord: any): Observable<ServerResponse> {
    ...
    this.loaded.next(true);
    return this.httpClient.put<ServerResponse>(url, photographicRecord)
      .pipe(
        map(response => response),
        tap(response => {
          this.loaded.next(false);
          const index = this.photographicRecordList.data.findIndex((photographicRecord: PhotographicRecordModel) => photographicRecord.id === response.data.id);
  ...
  deletePhotographicRecord(id: number): Observable<ServerResponse> {
    ...
    return this.httpClient.delete<ServerResponse>(url)
      .pipe(
        ...
        tap(response => {
          this.loaded.next(false);
          this.photographicRecordList.data = this.photographicRecordList.data.filter((photographicRecord: PhotographicRecordModel) => photographicRecord.id !== response.data.id);
          this.photographicRecords.next(this.photographicRecordList);
        },error => {
          this.loaded.next(false);
        }),
  ...
  deletePhotographicRecords(ids: (number|undefined)[]): Observable<ServerResponse> {
      ...
      .pipe(
        ...
        tap(response => {
          ...
          ids.forEach(photographicRecordId => {
            this.photographicRecordList.data = this.photographicRecordList.data.filter((photographicRecord: PhotographicRecordModel) => photographicRecord.id !== photographicRecordId);
          })
          this.photographicRecords.next(this.photographicRecordList);
        },error => {
          this.loaded.next(false);
  ...
  getFiles(recordId: number, paginator: PaginatorModel = {}, filter: string = ''): Observable<ServerResponse> {
  ...
  uploadFiles(recordId: number, data: FormData, params = new HttpParams()): Observable<ServerResponse> {
    const url = `${environment.API_URL_PRIVATE}/record/${recordId}/file`;
  ...
  uploadImageRecord(recordId: number, data: FormData, params = new HttpParams()): Observable<ServerResponse> {

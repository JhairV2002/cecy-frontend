...
export class ProfileInstructorCourseHttpService {
  ...
  public profileInstructorCourses$ =
    this.profileInstructorCourses.asObservable();
  ...
  storeProfileInstructorCourse(
    profileInstructorCourse: CourseProfileModel
  ): Observable<ServerResponse> {
    const url = `${this.API_URL_PRIVATE}/profileInstructorCourses`;

    this.loaded.next(true);
    return this.httpClient
      .post<ServerResponse>(url, profileInstructorCourse)
      .pipe(
        map((response) => response),
        tap(
          (response) => {
            ...
            this.profileInstructorCourses.next(
              this.profileInstructorCourseList
            );
          },
          (error) => {
            this.loaded.next(false);
  ...
  updateProfileInstructorCourse(
    ...
    this.loaded.next(true);
    return this.httpClient
      .put<ServerResponse>(url, profileInstructorCourse)
      .pipe(
        map((response) => response),
        tap(
          (response) => {
            this.loaded.next(false);
            const index = this.profileInstructorCourseList.data.findIndex(
              (profileInstructorCourse: CourseProfileModel) =>
                profileInstructorCourse.id === response.data.id
            );
  ...
  deleteProfileInstructorCourse(id: number): Observable<ServerResponse> {
    ...
    return this.httpClient.delete<ServerResponse>(url).pipe(
      ...
      tap(
        (response) => {
          this.loaded.next(false);
          this.profileInstructorCourseList.data =
            this.profileInstructorCourseList.data.filter(
              (profileInstructorCourse: CourseProfileModel) =>
                profileInstructorCourse.id !== response.data.id
            );
          this.profileInstructorCourses.next(this.profileInstructorCourseList);
  ...
  deleteProfileInstructorCourses(
        ...
        (response) => {
          ...
          ids.forEach((profileInstructorCourseId) => {
            this.profileInstructorCourseList.data =
              this.profileInstructorCourseList.data.filter(
                (profileInstructorCourse: CourseProfileModel) =>
                  profileInstructorCourse.id !== profileInstructorCourseId
              );
          });
          this.profileInstructorCourses.next(this.profileInstructorCourseList);
        },

...
export class RegistrationHttpService {
  ...
  enrollParticipant(
    ...
    this.loaded.next(true);
    return this.httpClient.put<ServerResponse>(url, registrationBody).pipe(
      map((response) => response),
      tap((response) => {
        this.loaded.next(false);
  ...
  getRegistration(registrationID: number): Observable<ServerResponse> {
  ...
  reEnroll(
    ...
    this.loaded.next(true);
    return this.httpClient.put<ServerResponse>(url, reason).pipe(
      map((response) => response),
      tap((response) => {
        this.loaded.next(false);
        // const index = this.participantsList.data?.findIndex((participant: ParticipantModel) => participant.id === response.data.id);
  ...
  nullifyRegistration(
    ...
    this.loaded.next(true);
    return this.httpClient.patch<ServerResponse>(url, registrationBody).pipe(
      map((response) => response),
      tap((response) => {
        this.loaded.next(false);
  ...
  uploadFile(
    ...
    this.loaded.next(true);
    return this.httpClient.put<ServerResponse>(url, registration).pipe(
      map((response) => response),
      tap(
        (response) => {
          this.loaded.next(false);
          const index = this.registrationsList.data.findIndex(
            (registration: RegistrationModel) =>
    ...
    return this.httpClient.patch<ServerResponse>(url, { ids }).pipe(
      ...
      tap(
        (response) => {
          this.loaded.next(false);
          ids.forEach((registrationId) => {
            this.registrationsList.data = this.registrationsList.data.filter(
              (registration: UserModel) => registration.id !== registrationId
            );
          });
          this.registrations.next(this.registrationsList);
        },
  ...
  loadDetail(id: number): Observable<RegistrationModel> {
    ...
    this.loaded.next(true);
    return this.httpClient.get<RegistrationModel>(url).pipe(
      map((response) => response),
      tap(
        (response) => {
          this.loaded.next(false);
  ...
  downloadReportRecordCompetitors(detailPlanificationId: number) {
    this.getReportRecordCompetitors(detailPlanificationId).subscribe(
      (response) => {
  ...
  storeRegisterStudent(
    registrationStudent: RegistrationStudentModel
  ): Observable<ServerResponse> {
    const url = `${this.API_URL_PRIVATE}/register-student`;
    return this.httpClient.post<ServerResponse>(url, registrationStudent).pipe(
      map((response) => {
  ...
  downloadRequirement(id: number, registrationRequirementId: number) {
    this.getRequirement(id!, registrationRequirementId!).subscribe(
      (response) => {
        const binaryData = [] as BlobPart[];
        binaryData.push(response as BlobPart);
        const filePath = URL.createObjectURL(
  ...
  getRequirement(
    id: number,
    registrationRequirementId: number,
    params = new HttpParams()
  ) {
  ...
  uploadDocumentRegistration(
    requistrationRequirementId: number,
    data: FormData,
    params = new HttpParams()
  ): Observable<ServerResponse> {
  ...
  getRegistrationsByDetailPlanificationAndParticipant(

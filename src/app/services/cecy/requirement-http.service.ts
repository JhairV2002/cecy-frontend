...
export class RequirementHttpService {
  ...
  storeRequirement(requirement: RequirementModel): Observable<ServerResponse> {
    const url = `${this.API_URL_PRIVATE}`;
    this.loaded.next(true);
    return this.httpClient.post<ServerResponse>(url, requirement).pipe(
      map((response) => response),
      tap(
        (response) => {
          this.loaded.next(false);
  ...
  updateRequirement(
    ...
    const url = `${this.API_URL_PRIVATE}/${id}`;
    this.loaded.next(true);
    return this.httpClient.put<ServerResponse>(url, requirement).pipe(
      map((response) => response),
      tap(
        (response) => {
          this.loaded.next(false);
          const index = this.requirementsList.data.findIndex(
            (requirement: RequirementModel) =>
              requirement.id === response.data.id
          );
  ...
  deleteRequirement(id: number): Observable<ServerResponse> {
    ...
    return this.httpClient.delete<ServerResponse>(url).pipe(
      ...
      tap(
        (response) => {
          this.loaded.next(false);
          this.requirementsList.data = this.requirementsList.data.filter(
            (requirement: RequirementModel) =>
              requirement.id !== response.data.id
          );
          this.requirements.next(this.requirementsList);

...
export class TopicHttpService {
  ...
  getTopics(page: number = 1, search: string = '', courseId: number): Observable<ServerResponse> {
  ...
  storeTopic(topics: TopicModel[], courseId: number): Observable<ServerResponse> {
    const url = `${this.API_URL_PRIVATE}/${courseId}/topics`;
    this.loaded.next(true);
    return this.httpClient.post<ServerResponse>(url, { topics })
      .pipe(
        map(response => response),
        tap(response => {
  ...
  updateTopic(id: number | undefined, topic: TopicModel[], courseId: number): Observable<ServerResponse> {
    const url = `${this.API_URL_PRIVATE}/${courseId}/topics/${id}`;
    this.loaded.next(true);
  ...
  updateTopics(topic: TopicModel[], courseId: number): Observable<ServerResponse> {
    const url = `${this.API_URL_PRIVATE}/${courseId}/topics/`;
    this.loaded.next(true);
    return this.httpClient.put<ServerResponse>(url, topic)
      .pipe(
  ...
  destroyTopic(id: number | undefined, courseId: number): Observable<ServerResponse> {
    const url = `${this.API_URL_PRIVATE}/${courseId}/topics/${id}`;
    this.loaded.next(true);
  ...
  destroysTopics(ids: (number | undefined)[], courseId: number): Observable<ServerResponse> {
  ...
  getFiles(topicId: number, paginator: PaginatorModel = {}, filter: string = ''): Observable<ServerResponse> {
  ...
  uploadFiles(topicId: number, data: FormData, params = new HttpParams()): Observable<ServerResponse> {
    const url = `${environment.API_URL_PRIVATE}/topic/${topicId}/file`;
  ...
  getTopicsPublic(page: number = 1, search: string = '', courseId: number): Observable<ServerResponse> {
